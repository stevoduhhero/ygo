	...
	for (var listKey = 0; listKey < listCount; listKey++) {
		...
		for (var i = 0; i < 2; i++) {
			...
			if (list === "field") {
				...
				for (var x = 0; x < 13; x++) { //13 zones including field and pendulums
...
Game.prototype.move = function(user, source, target) {
	...
	if (moveZone) {
		if (target.who === "you") {
			//card moving zones
			var cardCache = player.field[source.zone][source.slot];
			var tarZone = player.field[target.zone];
			player.field[source.zone].splice(source.slot, 1);
			...
			var tarZone = player.opp().field[target.zone];
			var zoneCache = player.field[source.zone];
			if (tarZone.length) return; //there's already something in the zone
			player.field[source.zone] = [];
			tarZone = zoneCache;
		...
		if (target.who === "opp" && target.list === "field") {
			var tarZone = player.opp().field[target.zone];
			var cardCache = player[source.list][source.slot];
			if (tarZone.length) return; //there's already something in the zone
			player[source.list].splice(source.slot, 1);
			tarZone.push({
				id: cardCache,
				pos: 0
			});
			...
			var cardCache = player[source.list][source.slot];
			if (source.list === "field") {
				cardCache = player[source.list][source.zone][source.slot];
				player[source.list][source.zone].splice(source.slot, 1);
			} else player[source.list].splice(source.slot, 1);
			...
			if (target.list === "deck" && (!target.pos || target.pos === 0)) {
				player[target.list].unshift(cardCache); //at the top of the deck
			} else {
				player[target.list].push(cardCache);
			}
			revealId = cardCache;
	...
	function strConstruct(info) {
		...
		return player[info.who]().player + ',' + info.list + ',' + str(info.slot) + ',' + str(info.zone) + ',' + str(info.pos);
	}
	var dataString = 'move|' + strConstruct(source) + '|' + strConstruct(target);
	if (revealId !== false) {
		//don't send reveal'd id to spectators
		player.send(dataString + '|' + revealId);
		player.sendExclude(dataSring);
		return;
	}
